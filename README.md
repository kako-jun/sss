...
- **4K対応**: 高解像度ディスプレイに最適化、自動リサイズ
- **base64画像ロード**: クロスプラットフォーム対応の画像読み込み
...
### バックエンド (Rust)
- **Tauri v1.5**: デスクトップアプリフレームワーク
- **rusqlite v0.31**: SQLiteデータベース
- **rayon v1.8**: 並列処理
- **image v0.24**: 画像処理
- **kamadak-exif v0.6**: EXIF情報読み取り
- **base64 v0.21**: 画像エンコーディング
...
- 💾 ファイルサイズ
- 📅 撮影日時（EXIFから、画像のみ）
- 📸 カメラ情報（EXIF: メーカー、モデル、焦点距離、F値、ISO、画像のみ）
